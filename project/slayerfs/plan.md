...
| --- | --- | ---: | --- | ---: | --- |
| `fio-randrw` read BW | `perf-run-1780191648-27152` | 120.86 MiB/s | `juicefs-perf-run-1780153510-28102` | 66.00 MiB/s | SlayerFS faster |
| `fio-randrw` write BW | `perf-run-1780191648-27152` | 56.01 MiB/s | `juicefs-perf-run-1780153510-28102` | 29.29 MiB/s | SlayerFS faster |
...
| Operation | SlayerFS `perf-run-1780191648-27152` | JuiceFS `juicefs-perf-run-1780153510-28102` | Gap |
| --- | ---: | ---: | --- |
| create | 178.0 ops/s | 285.1 ops/s | SlayerFS slower, still variable |
| open | 5794.6 ops/s | 6049.8 ops/s | Close to JuiceFS |
| stat | 1,100,979.1 ops/s | 1,101,680 ops/s | Similar |
| readdir | 31,160.5 ops/s | 37,075.7 ops/s | SlayerFS slower |
| rename | 948.3 ops/s | 1438.3 ops/s | SlayerFS slower, repeat-variable |
...
| `perf-run-1780190903-31660` | `SLAYERFS_FUSE_WORKERS=1 SLAYERFS_FUSE_MAX_BACKGROUND=64` dirperf-only experiment | 13s | n/a | First proof that low-overhead FUSE dispatch can match JuiceFS isolated `dirperf` |
...
| `cargo test -p slayerfs config::tests::mount_config_defaults_use_low_overhead_fuse_dispatch -- --nocapture` | Red/green verified: failed before the default change with host-derived workers, then passed with `fuse_workers=1` |
...
perf record -F 99 -e cpu-clock --call-graph fp -a -o /tmp/slayerfs-step13-perf/slayerfs-dirperf.data -- ./docker/compose-xfstests/run_redis_perf.sh --tools "dirperf"
SLAYERFS_FUSE_WORKERS=1 SLAYERFS_FUSE_MAX_BACKGROUND=64 ./docker/compose-xfstests/run_redis_perf.sh --tools "dirperf"
SLAYERFS_FUSE_WORKERS=1 SLAYERFS_FUSE_MAX_BACKGROUND=64 ./docker/compose-xfstests/run_redis_perf.sh --tools "dirperf metaperf"
SLAYERFS_FUSE_WORKERS=1 SLAYERFS_FUSE_MAX_BACKGROUND=64 ./docker/compose-xfstests/run_redis_perf.sh --tools "fio-randrw dirperf"
cargo test -p slayerfs config::tests::mount_config_defaults_use_low_overhead_fuse_dispatch -- --nocapture
...
System-wide perf around SlayerFS isolated dirperf: dirperf 14s.
...
PID-attached perf around the SlayerFS mount process: dirperf 15s.
...
FUSE override experiment: `SLAYERFS_FUSE_WORKERS=1 SLAYERFS_FUSE_MAX_BACKGROUND=64`.
...
Decision: keep the low-overhead default FUSE dispatch and the perf-runner override knobs. This matches JuiceFS on isolated `dirperf`, keeps fio read/write above the previous accepted SlayerFS baseline, and preserves the operator escape hatch for worker-pool concurrency. The remaining issue is not proven to be Redis Lua CPU; it is mixed-run variability plus slower create/readdir/rename operation rates.
