...
| `dirperf` wall time | `perf-run-1780174664-25456` | 18s | `juicefs-perf-run-1780153510-28102` | 13s | Improved, still not close |
| `metaperf` wall time | `perf-run-1780174664-25456` | 208s | `juicefs-perf-run-1780153510-28102` | 222s | Better on wall time |

Important caveat: `metaperf` wall time is close, but individual metadata ops still lag JuiceFS on create/open/rename. The latest `dirperf` gap is now 18s vs JuiceFS 13s; the next bottleneck is still metadata/FUSE hot-path overhead rather than disk-cache throughput.

Latest focused metadata detail:

| Operation | SlayerFS `perf-run-1780174664-25456` | JuiceFS `juicefs-perf-run-1780153510-28102` | Gap |
| --- | ---: | ---: | --- |
| create | 181.7 ops/s | 285.1 ops/s | SlayerFS slower |
| open | 2036.8 ops/s | 6049.8 ops/s | SlayerFS much slower |
| stat | 1,112,012 ops/s | 1,101,680 ops/s | Similar |
| readdir | 31,800.7 ops/s | 37,075.7 ops/s | SlayerFS slower, improved locally |
| rename | 937.8 ops/s | 1438.3 ops/s | SlayerFS slower, improved locally |
...
| `perf-run-1780174664-25456` | Same code after adding TTL cleanup to the recently-unlinked attr cache | 18s | 208s | Current accepted result; `dirperf` repeats at 18s and `metaperf` stays 208s |
...
- [ ] **Step 8: Consider remaining Redis single-EVAL equivalents**
...
- [ ] **Step 9: Consider parent dentry caching or batched create handling**
