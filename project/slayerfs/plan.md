...
| `dirperf` wall time | `perf-run-1780176122-15261` | 16s | `juicefs-perf-run-1780153510-28102` | 13s | Improved, closer |
| `metaperf` wall time | `perf-run-1780176122-15261` | 205s | `juicefs-perf-run-1780153510-28102` | 222s | Better on wall time |

Important caveat: `metaperf` wall time is close, but individual metadata ops still lag JuiceFS on create/open/rename. The latest `dirperf` gap is now 16s vs JuiceFS 13s; the next bottleneck is still metadata/FUSE hot-path overhead rather than disk-cache throughput.

Latest focused metadata detail:

| Operation | SlayerFS `perf-run-1780176122-15261` | JuiceFS `juicefs-perf-run-1780153510-28102` | Gap |
| --- | ---: | ---: | --- |
| create | 197.1 ops/s | 285.1 ops/s | SlayerFS slower, improved locally |
| open | 2814.3 ops/s | 6049.8 ops/s | SlayerFS slower, improved locally |
| stat | 1,112,907.5 ops/s | 1,101,680 ops/s | Similar |
| readdir | 32,228.3 ops/s | 37,075.7 ops/s | SlayerFS slower, improved locally |
| rename | 914.3 ops/s | 1438.3 ops/s | SlayerFS slower |
...
| `perf-run-1780176122-15261` | No-write flush/close skips timestamp metadata `SET` | 16s | 205s | Current accepted result; `dirperf` moves closer to JuiceFS 13s and `metaperf` improves from 208s |
...
| `timeout 20m cargo test -p slayerfs --lib --bins --tests -- --format terse` | Passed on rerun: lib target 290 passed/143 ignored, bin target 282 passed/143 ignored, integration tests passed with expected ignored external-service tests |
| `git diff --check` | Passed |
