...
修改 `rfuse3/src/raw/session/mod.rs`，在 FUSE init 回复中无条件设置 `FUSE_FLOCK_LOCKS`。
...
在当前环境（Linux 6.17 + rfuse3 0.0.8 + mount helper）中，即使设置了标志，锁也未出现。可能原因：

- mount helper 路径下 FUSE init 回复序列化时机问题
- 内核 FUSE 驱动对 flock 的 `/proc/locks` 可见性不完整
- rfuse3 非标准 mount 路径下 `init_out.flags` 构建方式差异

## 结论
FUSE 内核/协议层面限制，不影响 slayerfs 正确性。已加入 `xfstests_slayer.exclude`。

---

## generic/632 补充
`generic/632` 要求 `/mnt/slayerfs` 为 shared mountpoint。FUSE 挂载默认是 private，不支持挂载传播（mount propagation）。同样为内核层面限制，已加入 exclude。
