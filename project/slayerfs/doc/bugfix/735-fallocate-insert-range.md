...
`xfs_io finsert` 调用 `fallocate(fd, FALLOC_FL_INSERT_RANGE, offset, length)`。slayerfs 的 fallocate 实现（`fuse/mod.rs`, `vfs/fs/mod.rs`）只支持 `mode == 0`（普通空间分配）：
...
这涉及元数据层的数据块重映射，对于基于对象存储的 slayerfs 来说非常复杂——每个 chunk 的 slice 偏移需要重新计算。
