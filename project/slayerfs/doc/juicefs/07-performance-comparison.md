# SlayerFS vs JuiceFS 性能对比

> 环境: rustfs S3 + Redis, io_uring, 4M block
> JuiceFS 数据来自 `juicefs-perf-run-1779469615` (全 11/11 通过)
> SlayerFS 数据来自 `perf-run-1779435359` (全 11/11 通过)

## FIO 吞吐量

| Test | rw | jobs | JuiceFS BW | SlayerFS BW | Ratio | JuiceFS P99 | SlayerFS P99 |
|------|----|------|-----------|------------|-------|------------|-------------|
...
- SlayerFS randrw P99 高达 9.2s, JuiceFS 仅 1.1s

## 元数据性能 (metaperf)

| Operation | JuiceFS ops/s | SlayerFS ops/s | Ratio | 说明 |
|-----------|-------------|---------------|-------|------|
