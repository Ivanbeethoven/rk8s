...
SlayerFS Rust 侧当前核心目录：

| SlayerFS 模块 | 文件数 | 主要职责 |
|---|---:|---|
...
| 能力域 | SlayerFS | JuiceFS | 差异 |
|---|---|---|---|
| CLI 入口 | `src/main.rs`、`src/config.rs` | `cmd/*.go`、`cmd/main.go` | SlayerFS 当前命令面很窄；JuiceFS 的命令即产品控制面 |
| 元数据抽象 | `src/meta/store.rs` | `pkg/meta/interface.go` | 两者都很大；SlayerFS 许多方法仍是默认未实现 |
| 元数据实现 | `src/meta/stores/{database,redis,etcd}` | `pkg/meta/{redis,sql,tkv,...}` | JuiceFS 后端更广，且围绕同一语义长期打磨 |
| 元数据客户端缓存 | `src/meta/client/*` | `pkg/meta/base.go`、openfile/cache 相关 | SlayerFS 有 inode/path cache 与 Etcd watch；JuiceFS 有 open-cache、session、quota、ACL 等完整整合 |
| 数据布局 | `src/chunk/layout.rs`、`slice.rs` | `pkg/meta/interface.go` constants、`pkg/chunk` | 默认 64MiB chunk 类似；对象 key 和 slice 生命周期细节不同 |
| 对象存储抽象 | `src/cadapter/client.rs`、`src/chunk/store.rs` | `pkg/object/interface.go` | SlayerFS 抽象简洁；JuiceFS object interface 包含 List/Head/Multipart/Restore 等生产能力 |
| 读写路径 | `src/vfs/io/{reader,writer}.rs` | `pkg/vfs/{reader,writer}.go`、`pkg/chunk/cached_store.go` | SlayerFS 状态机清晰但仍需语义验证；JuiceFS 缓存/限速/回写成熟 |
| FUSE | `src/fuse/*` 使用 `rfuse3` | `pkg/fuse/*` | SlayerFS FUSE 层薄；JuiceFS mount option 与平台行为更多 |
| 后台维护 | `src/chunk/compact/*`、`src/control/*` | `cmd/gc.go`、`cmd/compact.go`、`pkg/vfs/compact.go`、`pkg/meta` cleanup | SlayerFS 已有 compact/gc 内核，但 CLI 与可观测性不够 |
| K8s | `operator/slayerfs-operator` | JuiceFS CSI 独立生态 | SlayerFS operator 是雏形；CSI 能力缺失 |

## 架构边界差异

### SlayerFS 更偏“库内强类型分层”

SlayerFS 的抽象非常 Rust 化：
...
它的好处是：每个元数据后端最终要向同一个产品语义收敛。缺点是接口大、实现复杂。SlayerFS 已经开始走类似路线，但还没有完成“所有后端都承诺同一语义”的阶段。
...
JuiceFS 的 `meta.Format` 记录 name、UUID、storage、bucket、block size、compression、hash prefix、capacity、inodes、encryption、trash days、quota、ACL、client version 等。SlayerFS 当前主要通过 mount config 拼出运行参数，缺少“格式化后的卷不变量”。

建议新增：

- `slayerfs format <meta-url> <name>`
- `volume_format` 元数据记录
...
SlayerFS control plane 目前支持 `Ping/GetInfo/RunGc/GetJob`。建议将 compact、cache warmup/evict、stats、debug dump、session list、fsck 都纳入同一异步 job 框架。
