...
- SlayerFS：`src/vfs/*`、`src/fuse/*`、`src/posix.rs`
- JuiceFS：`juicefs/pkg/vfs/*`、`juicefs/pkg/fuse/*`

## 总体判断

SlayerFS 的 VFS 结构已经比较完整：有 inode runtime、handle registry、reader/writer、append locks、modified tracker、background tasks，并通过 rfuse3 暴露 FUSE。差距主要不在“有没有函数”，而在 POSIX 边界是否被完整定义、跨客户端是否一致、错误是否按系统调用语义传播。

## 已有能力

SlayerFS 当前 VFS/Meta 层已覆盖或正在覆盖：
...
JuiceFS 对 close-to-open、缓存例外、open-cache 等边界有长期实践。SlayerFS 文档中也提到跨客户端缓存一致性仍是弱项。
...
对象存储文件系统最容易出错的是“用户以为落盘，实际只是进了后台队列”。SlayerFS 写路径有异步上传和 commit，因此必须规定：
...
POSIX `rename(old, new)` 在 `new` 是普通文件时应原子替换。SlayerFS 历史 bug 文档已提到相关风险。JuiceFS `Rename` 接口还支持 flags，包括 no-replace/exchange/whiteout 等语义。
...
SlayerFS 当前 writer 有 per-handle gate、append locks 和 flush 等机制，但 truncate 与 in-flight write 的全局顺序必须非常明确。
...
JuiceFS `Access`、`Open`、`CheckSetAttr`、ACL 等在 meta 层形成统一入口。SlayerFS 目前权限能力正在补，但需要避免每个 VFS 操作自行判断。
...
JuiceFS 有 readdir cache、readdir plus、entry timeout 等配置。SlayerFS 有 batch prefetch 和 dir handle，但需要明确：
...
SlayerFS 使用 `rfuse3`，FUSE 层较薄，优势是复杂度低。JuiceFS mount 侧包含大量 mount options：
...
SlayerFS 当前暴露的核心是 data/meta/layout/fuse workers/max-background。

提升方向：

- 逐步增加面向语义和性能的 mount options，而不是只暴露底层参数。
- 将选项写入 `slayerfs info/status`，便于排查。
- FUSE 错误码建立映射测试，避免内部错误都变成 EIO。

## 文件锁差异

SlayerFS 已有 flock/plock 数据结构和测试文档。JuiceFS 的 session 会携带锁信息，并在 stale session cleanup 时处理。
