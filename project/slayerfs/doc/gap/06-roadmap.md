...
2. 新增 `slayerfs format`，写入最小 volume format：name、uuid、chunk_size、block_size、data_backend、feature flags。
...
8. `slayerfs status` 读取 control plane，展示 mount/session/jobs。
9. `slayerfs stats` 输出 JSON，先覆盖 meta/object/cache/writer 基础计数。
