...
SlayerFS 已有 Unix socket control plane：
...
JuiceFS 有 Prometheus 指标、profile/debug 命令、pprof、日志与 status。SlayerFS 当前主要依赖 tracing/log、可选 profiling feature、测试脚本产物。
...
JuiceFS 有较成熟的 release、version、兼容控制。SlayerFS 需要：
