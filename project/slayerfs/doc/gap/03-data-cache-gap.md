...
- SlayerFS：`src/chunk/*`、`src/vfs/io/*`、`src/vfs/cache/*`、`src/cadapter/*`
...
SlayerFS 当前默认 block 为 4MiB，并用 `SliceDesc { slice_id, chunk_id, offset, length }` 描述一次写入在 chunk 内的逻辑范围。对象 key 当前是：
...
- 将 SlayerFS 写入语义整理成明确状态表：返回给用户前至少满足什么条件。
...
- 将 SlayerFS `ChunksCache`、page cache、prefetch 的配置暴露到 mount/config。
...
JuiceFS 有 format 级 compression 和 object encryption。SlayerFS 当前未看到完整数据面压缩/加密闭环。
...
JuiceFS 的 compact/gc 已和 CLI、status、fsck、object listing、trash/session 清理结合得更完整。SlayerFS 目前更像“后台机制已存在，但产品入口和可观测性不足”。

提升方向：

- `slayerfs compact <path>`：手动触发并展示进度。
- `slayerfs gc --dry-run/--delete`：列出 orphan slice/object 明细。
- `slayerfs fsck`：对账 meta slices 与 object blocks。
