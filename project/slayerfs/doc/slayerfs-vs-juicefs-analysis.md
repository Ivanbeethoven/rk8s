...
SlayerFS 性能上限、延迟稳定性和生产可调优性的实现差异。

## 1. 总体结论

SlayerFS 的写路径已经具备较强的异步 pipeline 特征：Rust/tokio、4MiB FUSE max_write、
进程级上传并发、按 chunk 提交、writeback-cache 热路径优化、读前 dirty overlay 等设计，
在大块顺序写和 mmap/writeback 场景上有潜力。

但与 JuiceFS 相比，SlayerFS 当前仍存在几个高影响性能缺口：
...
7. FUSE 和 VFS 预读策略没有形成一套闭环，SlayerFS 自己有激进 prefetch，但 FUSE 层未显式设置 max_readahead。

这些问题中，最值得优先处理的是：磁盘读缓存、压缩、统一预算、前后台 I/O 优先级，以及 slice 聚合/合并。

## 2. 核心差异总览

| 维度 | SlayerFS 当前实现 | JuiceFS 当前实现 | 对 SlayerFS 的影响 |
|---|---|---|---|
| FUSE 库 | Rust `rfuse3`，`write_back(true)`，`max_write=4MiB` | Go `go-fuse`，`MaxReadAhead=1MiB`，Linux/FreeBSD 参数不同 | SlayerFS 单次写入更大，但 FUSE readahead 未显式配置，读预取更多依赖用户态 |
| FUSE 并发 | 默认 `fuse_max_background=512`，session worker 可配 | Linux 默认 `MaxBackground=50`，FreeBSD 路径 200 | SlayerFS 并发上限更激进，容易暴露后端和内存预算不足 |
| 写语义 | 默认 `UploadBeforeCommit`，可用环境变量切到 `CommitBeforeUpload` | 普通模式上传后提交，writeback 可 stage 到磁盘后延迟上传 | SlayerFS 默认更安全，但 commit_first 模式会带来跨客户端读空洞风险 |
| 写缓存 | 内存 dirty slice + SSD write-back cache 雏形 | 成熟 staging/writeback，延迟上传、扫描、失败处理更完整 | SlayerFS 已有方向，但缺少完整运维和恢复闭环 |
| 读缓存 | block memory cache、64KiB page cache、reader slice cache、全局 prefetcher | page/chunk cache + disk cache + singleflight + warmup/evict/check | SlayerFS 热读主要依赖内存，跨重启和大数据集随机读弱 |
| 压缩 | 未看到数据面压缩闭环 | format 级 `lz4`/`zstd`/`none`，PUT 前压缩、GET 后解压 | SlayerFS 在可压缩数据上浪费带宽和对象存储时间 |
| 限流 | 上传全局 `UPLOAD_SEM=256`，无下载限流和带宽限流 | `MaxUpload`、`MaxDownload`、`UploadLimit`、`DownloadLimit` | SlayerFS 容易在高并发时打满对象存储、网络或本机 FD |
| 读前一致性 | 不主动 flush，先读 committed，再 overlay dirty | `VFS.Read()` 中先 `writer.Flush(ctx, ino)` | SlayerFS 延迟更低，但一致性和 cache 失效更复杂 |
| 元数据写 | Redis `write()` 用 Lua 合并 append slice + extend size；get_slices 可缓存 | 元数据后端成熟，格式/配额/ACL/session 等能力完整 | SlayerFS 已做 RTT 优化，但 slice 数过多仍会放大读和 compaction 成本 |
| 可观测性和运维 | `.stats`、文档和部分 tracing 已有 | Prometheus、cache 管理、debug/warmup/gc/bench 更完整 | SlayerFS 定位性能瓶颈仍依赖手工 tracing 和专项脚本 |
...
1. FUSE writeback-cache 使内核可以发起 `FUSE_WRITE_CACHE` 和 O_WRONLY 上的补页读，SlayerFS 为此在 `read()` 中临时 open 读 handle。这会给 partial page write 和 mmap 场景带来额外 stat/open/close 成本。
2. `max_write=4MiB` 对顺序写有利，但对高并发小写没有帮助，瓶颈会转移到 slice 数量、metadata append 和 upload 调度。
3. SlayerFS 未在 mount options 中显式设置 `max_readahead`，而 JuiceFS 直接设置 `MaxReadAhead=1MiB`。SlayerFS 的用户态 prefetch 很激进，但内核层预读窗口可能没有被充分打开。
...
这些优化说明 SlayerFS 的写路径已经不是简单 MVP。当前主要风险在于：
...
JuiceFS 的写路径吞吐不一定在每个微基准上都比 SlayerFS 激进，但它的生产控制面更完整：限流、压缩、staging、延迟上传、重试、cache 管理和观测指标是成体系的。
...
因此 JuiceFS 的随机读、重复读、跨进程热读更依赖磁盘 cache 和限流，而 SlayerFS 当前更多依赖内存与短期 page cache。

## 6. 元数据与 slice 组织差异

SlayerFS 和 JuiceFS 都使用 64MiB chunk、slice 覆盖语义、对象 block 存储。SlayerFS 当前的积极点：
...
- SlayerFS 用户态 `max_ahead=32MiB`，GlobalPrefetcher 并发 64。
...
SlayerFS 当前的优势在写路径并发和 mmap/writeback 热路径优化，部分微基准可能已经比保守配置下的 JuiceFS 更激进。但 JuiceFS 的优势在产品化的 cache、压缩、限流、format 契约、运维工具和长期稳定性。

如果 SlayerFS 的目标是接近 JuiceFS 的生产性能，需要优先把“单点优化”收敛成“全局资源治理”：统一预算、分级缓存、压缩、限流、可观测性和自适应策略。否则在单客户端顺序写之外，随机读、混合负载、小写放大和 compaction 干扰仍会成为主要性能瓶颈。
