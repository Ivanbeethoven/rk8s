...
`generic/075` 在 xfstests 中概率性卡死。排查发现 `slayerfs.log` 中 Redis 连接数在 2 秒内达到 170 条，大量重复的 `redis connection established` 日志。根因是 chunk slice list 的并发修改使用了 `WATCH + MULTI + EXEC`，每次重试都调用 `create_connection` 新建一条 TCP 连接。在 fsx 两进程并发 truncate 的高冲突场景下，WATCH 频繁失败 → 重试 → 新建连接，形成连接风暴，最终 Redis 被打满、文件系统卡死。
