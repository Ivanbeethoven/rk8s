...
**JuiceFS 源码参照:**
- `/mnt/rk8s/project/slayerfs/juicefs/pkg/vfs/reader.go:162-232` — slice 读重试
- `/mnt/rk8s/project/slayerfs/juicefs/pkg/vfs/reader.go:652-659` — tail prefetch
- `/mnt/rk8s/project/slayerfs/juicefs/pkg/chunk/disk_cache_state.go` — 磁盘健康状态机
- `/mnt/rk8s/project/slayerfs/juicefs/pkg/chunk/disk_cache.go:436-472` — insert 去重
- `/mnt/rk8s/project/slayerfs/juicefs/pkg/chunk/cache_eviction.go` — 淘汰策略
...
SlayerFS 当前 `insert_opportunistic` 会在 hot_cache insert 后无条件 spawn disk write。多 reader/prefetcher 并发对同 key 调用会导致重复磁盘写。

**实现:**

- [ ] **Step 1: 添加 `disk_insert_inflight: DashSet<String>` 到 ChunksCache**

在 `slayerfs/src/chunk/cache.rs` ChunksCache 结构体:
```rust
use dashmap::DashSet;
...
**对标 JuiceFS:** `cache_eviction.go` — 2-random 淘汰 + atime 比较。JuiceFS 不做频率感知晋升，但 SlayerFS 的 Policy 结构可以做得更好。
...
在 `slayerfs/src/chunk/cache.rs` Policy::new():
```rust
...
3. 最大重试次数 `maxRetries` (默认 50, SlayerFS 用 3-5 即可)

**实现:**

- [ ] **Step 1: 在 `background_fetch` 外围包 retry loop**

在 `slayerfs/src/vfs/io/reader.rs:378-454`:
```rust
...
在 `slayerfs/src/vfs/io/reader.rs` `prepare_ahead_slices` 方法:
```rust
...
SlayerFS 版本简化: 2 态 (Normal / Bypassed)，省略 Unstable 探测阶段。

**实现:**

- [ ] **Step 1: 创建 `slayerfs/src/chunk/cache_health.rs`**
