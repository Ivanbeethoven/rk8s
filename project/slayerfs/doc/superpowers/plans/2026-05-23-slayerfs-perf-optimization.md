# SlayerFS Performance Optimization Plan
...
**Tech Stack:** Rust, tokio, moka cache, Redis, S3 API, FUSE3 (rfuse3).

**Analysis based on:** `doc/juicefs/` documentation (7 files) and thorough SlayerFS source code review.
...
**Files:**
- Modify: `slayerfs/src/vfs/fs/mod.rs:345-355`
- Modify: `slayerfs/src/vfs/cache/lru_cache.rs` (keep file, mark as dead)
- Modify: `slayerfs/src/vfs/cache/read_cache.rs` (keep trait, mark as unused)
...
```bash
cd /mnt/rk8s/project/slayerfs && rg "get_block|put_block|remove_block|ReadCache|LruReadCache" slayerfs/src/ --no-filename
```
...
Edit `slayerfs/src/vfs/fs/mod.rs`, remove lines ~345-355:
```rust
...
```
```bash
cd /mnt/rk8s/project/slayerfs && cargo check -p slayerfs 2>&1 | head -20
```
...
- [ ] **Step 5: Commit**
```bash
git add slayerfs/src/vfs/fs/mod.rs slayerfs/src/vfs/cache/
...
**Files:**
- Modify: `slayerfs/src/meta/client/cache.rs:94-102` (InodeCache)
- Modify: `slayerfs/src/meta/stores/redis/mod.rs:1151-1153` (node_cache TTL)
- Modify: `slayerfs/src/meta/client/mod.rs:884-903` (cached_stat path)
- Modify: `slayerfs/src/vfs/fs/mod.rs` (FUSE attr timeout)
...
Edit `slayerfs/src/meta/stores/redis/mod.rs:1151-1153`:
```rust
// Before:
...
```bash
cd /mnt/rk8s/project/slayerfs && rg -n "format.*\bi\b.*\{" slayerfs/src/meta/stores/redis/mod.rs | head -20
```

- [ ] **Step 3: Extend FUSE kernel attribute cache timeout**
Edit `slayerfs/src/vfs/fs/mod.rs` — find where FUSE `attr_timeout` is set (likely in the mount options or `getattr` response). Set to 1.0s (matching JuiceFS default):
```rust
...
Find the `open()` handler in `slayerfs/src/vfs/fs/mod.rs` and ensure it populates the `InodeCache`:
```rust
...
**Files:**
- Create: `slayerfs/src/chunk/cache_integrity.rs`
- Modify: `slayerfs/src/chunk/cache.rs:362-381` (DiskStorage::load — add CRC32C verify)
- Modify: `slayerfs/src/chunk/cache.rs:260-290` (DiskStorage::store — add CRC32C write)
- Modify: `slayerfs/src/chunk/store.rs:478-481` (async cache population)

**Context:** `ChunksCache::DiskStorage` reads cache files with no integrity check (line 362-381: `tokio::fs::read(filepath)`). JuiceFS writes CRC32C checksums per 32KB block and verifies on read. SlayerFS must add this to prevent silent data corruption. Cache file format needs to change from raw data to `[data][checksums]`.

- [ ] **Step 1: Define cache file format**
In `slayerfs/src/chunk/cache_integrity.rs`:
```rust
use crc32c::crc32c;
...
In `slayerfs/src/chunk/cache.rs`, modify `DiskStorage::store()` (~line 260):
```rust
...
In `slayerfs/src/chunk/cache.rs`, modify `DiskStorage::load()` (~line 362):
```rust
...
In `slayerfs/src/chunk/store.rs:478-481`, change synchronous cache insert to spawn background:
```rust
// Before:
...
- [ ] **Step 6: Run tests**
```bash
cd /mnt/rk8s/project/slayerfs && cargo test -p slayerfs -- chunk::cache_integrity
```
Expected: All tests pass.

- [ ] **Step 7: Commit**
```bash
git add slayerfs/Cargo.toml slayerfs/src/chunk/
...
**Files:**
- Modify: `slayerfs/src/chunk/store.rs:389-446` (small-range read path)
...
```bash
cd /mnt/rk8s/project/slayerfs && rg "compression" slayerfs/src/chunk/store.rs | head -10
```
...
In `slayerfs/src/chunk/store.rs:410-419`, after `get_object_range`:
```rust
let read_len = client
...
- [ ] **Step 5: Commit**
```bash
git add slayerfs/src/chunk/store.rs
...
**Files:**
- Modify: `slayerfs/src/chunk/store.rs:147` (add page-level SingleFlight)
- Modify: `slayerfs/src/chunk/store.rs:389-446` (page read path)
...
**Files:**
- Modify: `slayerfs/src/chunk/store.rs:440-446` (after page cache insert, trigger prefetch)
- Modify: `slayerfs/src/vfs/cache/prefetch.rs:56-149` (ensure prefetcher can handle block keys)
...
**Files:**
- Modify: `slayerfs/src/vfs/cache/prefetch.rs:143`
...
**Files:**
- Modify: `slayerfs/src/vfs/io/writer.rs:266-270` (has_idle_block)
- Modify: `slayerfs/src/vfs/io/writer.rs:1485-1526` (spawn_upload_task)
...
**Files:**
- Modify: `slayerfs/src/vfs/io/reader.rs:693-703` (prepare_ahead_slices)
...
**Files:**
- Modify: `slayerfs/src/vfs/io/writer.rs:579-640` (try_commit)
- Modify: `slayerfs/src/chunk/compact/worker.rs:177-274`
...
**Files:**
- Modify: `slayerfs/src/vfs/io/writer.rs:1733-1764` (commit_chunk)
...
**Files:**
- Modify: `slayerfs/src/vfs/cache/write_back.rs:60-212`
- Modify: `slayerfs/src/vfs/cache/config.rs`
