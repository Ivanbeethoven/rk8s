...
```bash
SLAYERFS_S3_ENDPOINT=http://127.0.0.1:9000 \
SLAYERFS_S3_BUCKET=slayerfs-data \
SLAYERFS_S3_REGION=us-east-1 \
AWS_ACCESS_KEY_ID=rustfsadmin \
AWS_SECRET_ACCESS_KEY=rustfsadmin \
AWS_EC2_METADATA_DISABLED=true \
cargo test -p slayerfs --lib rustfs_small_object_streaming_body_compat -- --ignored --nocapture
```

The test directly exercises the S3 adapter against a live RustFS instance. It does not use SlayerFS VFS, FUSE, xfstests, Redis, or the writer pipeline.
...
The stronger explanation is a SlayerFS writer progress bug: a slice can become frozen while the commit thread waits for upload completion, but no upload task is guaranteed to make progress for that slice.

Old hang logs repeatedly stopped here:

```text
slayerfs::vfs::io::writer::commit_chunk.wait_upload
...
- The observed xfstests hang is caused by SlayerFS writer coordination around frozen slices and upload progress.
