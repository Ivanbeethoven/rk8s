...
SlayerFS 提供了多层性能分析和监控手段：运行时统计（.stats 虚拟文件）、分布式追踪（tracing）、CPU 火焰图、tokio 异步任务分析和堆内存分析。
...
```
slayerfs_uptime_seconds 3600
slayerfs_fuse_read_ops_total 123456
slayerfs_fuse_read_bytes_total 1073741824
slayerfs_fuse_read_lat_us_total 5000000
slayerfs_fuse_write_ops_total 45678
slayerfs_fuse_write_bytes_total 536870912
slayerfs_fuse_write_lat_us_total 8000000
slayerfs_fuse_lookup_ops_total 789012
...
```
...
| `SLAYERFS_BENCH_BLOCK_MB` | 1 | 单次 IO 块大小 (MiB) |
| `SLAYERFS_BENCH_BIG_FILE_MB` | 512 | 大文件逻辑大小 (MiB) |
| `SLAYERFS_BENCH_SMALL_FILE_KB` | 128 | 小文件大小 (KiB) |
| `SLAYERFS_BENCH_SMALL_FILE_COUNT` | 100 | 每线程小文件数 |
| `SLAYERFS_BENCH_SAMPLE_SIZE` | ≥10 | Criterion 样本数 |
| `SLAYERFS_BENCH_MODE` | direct | `direct` 直连 VFS，`fuse` 通过 FUSE |
| `SLAYERFS_BENCH_BACKEND` | local | `local` 或 `s3` |
| `SLAYERFS_BENCH_META_BACKEND` | sqlx | `sqlx`、`redis`、`etcd` |
| `SLAYERFS_BENCH_FLAMEGRAPH` | 未设置 | 设任意值采集火焰图 |
| `SLAYERFS_BENCH_CHROME` | 未设置 | Chrome trace 输出路径 |
...
产物：
- `/tmp/slayerfs-perf/flame/oncpu-flame.svg`
- `/tmp/slayerfs-perf/flame/offcpu-flame.svg`
- `/tmp/slayerfs-perf/results/fio-*.json`
- `/tmp/slayerfs-perf/llm-report.txt`
...
# 记录 perf 数据
sudo perf record -F 99 -g -- ./target/release/slayerfs mount /mnt/slayerfs ...

# 另一个终端运行工作负载
fio --directory=/mnt/slayerfs --rw=read --size=4G --bs=1M --direct=1 ...
...
输出：`target/criterion/slayerfs_big_file/write/1/profile/flamegraph.svg`
...
sudo RUST_LOG=slayerfs::vfs::io::reader=trace --preserve-env=_RJEM_MALLOC_CONF \
  _RJEM_MALLOC_CONF="prof:true,prof_active:true,lg_prof_interval:30,lg_prof_sample:21,prof_prefix:/tmp/slayerfs" \
  ./target/release/slayerfs mount /mnt/slayerfs ...
```

生成报告：
```bash
jeprof --show_bytes --svg target/release/slayerfs /tmp/slayerfs_heap.*.heap > /tmp/slayerfs_heap.svg
```
...
# 分级控制
RUST_LOG=slayerfs::vfs::io::reader=trace,slayerfs::chunk=debug,slayerfs=info
```
...
```bash
# FUSE 操作日志单独输出
SLAYERFS_FUSE_LOG_FILE=/var/log/slayerfs/fuse.log

# 主日志输出到文件
SLAYERFS_LOG_FILE=/var/log/slayerfs/slayerfs.log
```

FUSE 操作日志仅包含 `rfuse3::raw::logfs` 的 TRACE 级别事件（每个 FUSE 请求/响应的详细信息），主日志输出其余所有事件。两者互不重复。

## 工具速查

| 工具 | 适用场景 | 是否需编译特性 |
|---|---|---|
| `slayerfs-stats` | 实时吞吐/延迟监控 | 否 |
