...
SlayerFS 是单二进制程序，通过 CLI 参数、YAML 配置文件和环境变量控制所有行为。
...
| 命令 | 说明 |
|---|---|
| `slayerfs mount [OPTIONS] [MOUNT_POINT]` | FUSE 挂载 |
| `slayerfs gc [MOUNT_POINT] [--dry-run]` | 触发 GC（与运行中的 daemon 通信） |
| `slayerfs info [MOUNT_POINT]` | 查看挂载信息 |

### mount 参数

```
slayerfs mount [MOUNT_POINT] \
...
meta:
  ...
  sqlx:
    url: "sqlite:///var/lib/slayerfs/meta.db"
  # 或:
  # etcd:
  #   urls:
  #     - http://10.0.0.1:2379
...
```bash
sudo slayerfs mount /mnt/slayerfs --privileged ...
```
...
| 变量 | 说明 |
|---|---|
| `RUST_LOG` | 日志级别控制（`slayerfs=info`, `slayerfs=trace` 等） |
| `SLAYERFS_LOG_FILE` | 主日志输出文件路径 |
| `SLAYERFS_FUSE_LOG_FILE` | FUSE 操作日志单独输出路径 |
| `SLAYERFS_TRACE_CHROME` | Chrome trace 输出路径 |
| `SLAYERFS_TRACE_FLAME` | tracing-flame 输出路径 |
| `TOKIO_CONSOLE` | 设为任意值启用 tokio-console |
| `SLAYERFS_NOFILE_LIMIT` | 自定义 nofile 限制（默认 1,048,576） |
...
运行中的 slayerfs 进程通过 Unix domain socket 提供控制面接口：

```
/run/slayerfs/<mount_hash>/control.sock
```

`slayerfs gc` 和 `slayerfs info` 命令通过此 socket 与 daemon 通信：

```bash
# 查看挂载信息
slayerfs info /mnt/slayerfs

# 触发 GC (扫描)
slayerfs gc /mnt/slayerfs --dry-run

# 触发 GC (实际删除)
slayerfs gc /mnt/slayerfs
```
