#!/usr/bin/env python3
"""SlayerFS flame graph analysis helpers."""
import argparse
import re
import subprocess
import sys
...
def cmd_offcpu(perf_data: str, folded_out: str) -> None:
        ...
        if line.startswith("\t"):
            ...
            if header_m:
                if current_stack and any(
                    "target/release/slayerfs" in f or "slayerfs" in f
    ...
    print(f"  slayerfs off-CPU stacks: {len(offcpu):,}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SlayerFS flame graph analysis")
