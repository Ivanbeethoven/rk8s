//! `slayerfs-stats` — Real-time performance monitor for SlayerFS.
//!
...
//!
//! Usage:
//!   slayerfs-stats /mnt/slayerfs          # default 1s interval
//!   slayerfs-stats /mnt/slayerfs -i 2     # 2s interval
...
/// Real-time SlayerFS performance stats viewer.
#[derive(Parser, Debug)]
#[command(name = "slayerfs-stats", about = "Monitor SlayerFS performance in real-time")]
struct Args {
    /// Mount point of SlayerFS
    mountpoint: PathBuf,
    ...
    vec![
        Section {
            ...
            compute: |prev, curr, dt| {
                let read_ops = delta(prev, curr, "slayerfs_fuse_read_ops_total");
                let write_ops = delta(prev, curr, "slayerfs_fuse_write_ops_total");
                let lookup_ops = delta(prev, curr, "slayerfs_fuse_lookup_ops_total");
                let getattr_ops = delta(prev, curr, "slayerfs_fuse_getattr_ops_total");
                let open_ops = delta(prev, curr, "slayerfs_fuse_open_ops_total");
                let total_ops = read_ops + write_ops + lookup_ops + getattr_ops + open_ops;

                let read_bytes = delta(prev, curr, "slayerfs_fuse_read_bytes_total");
                let write_bytes = delta(prev, curr, "slayerfs_fuse_write_bytes_total");

                let read_lat = delta(prev, curr, "slayerfs_fuse_read_lat_us_total");
                let write_lat = delta(prev, curr, "slayerfs_fuse_write_lat_us_total");
        ...
        Section {
            ...
            compute: |prev, curr, dt| {
                let ops = delta(prev, curr, "slayerfs_meta_ops_total");
                let txn = delta(prev, curr, "slayerfs_meta_txn_ops_total");
                let lat = delta(prev, curr, "slayerfs_meta_lat_us_total");
                vec![
        ...
        Section {
            ...
            compute: |prev, curr, dt| {
                let get_ops = delta(prev, curr, "slayerfs_s3_get_ops_total");
                let get_bytes = delta(prev, curr, "slayerfs_s3_get_bytes_total");
                let put_ops = delta(prev, curr, "slayerfs_s3_put_ops_total");
                let put_bytes = delta(prev, curr, "slayerfs_s3_put_bytes_total");
                let del_ops = delta(prev, curr, "slayerfs_s3_del_ops_total");
                vec![
        ...
        Section {
            ...
            compute: |prev, curr, dt| {
                let hits = delta(prev, curr, "slayerfs_cache_hits_total");
                let misses = delta(prev, curr, "slayerfs_cache_misses_total");
                let dirty = curr.get("slayerfs_buffer_dirty_bytes").unwrap_or(&0);
                vec![
...
fn main() {
    ...
    if !stats_path.exists() {
        eprintln!(
            "Error: {} not found.\nMake sure SlayerFS is mounted at {:?}",
            stats_path.display(),
            args.mountpoint
        );
        std::process::exit(1);
    ...
    println!(
        "SlayerFS Stats — {} (interval: {}s, Ctrl+C to quit)\n",
