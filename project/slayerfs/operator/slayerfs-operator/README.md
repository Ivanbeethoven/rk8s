...
这是一个独立于 `slayerfs` 主二进制的 Kubernetes operator 工程。

当前这版 operator 仍然不是完整 CSI，但已经拆成两层 CR：

- `SlayerFSCluster`：管理 Redis、RustFS 和 SlayerFS 配置
- `SlayerFSMount`：专门管理挂载工作负载

详细文档入口：

- `docs/README.md`
- `docs/architecture.md`
- `docs/slayerfscluster.md`
- `docs/slayerfsmount.md`
- `docs/workflows.md`

## 当前范围

`SlayerFSCluster` 的 reconcile 逻辑负责创建和维护：
...
- SlayerFS 配置 `ConfigMap`

`SlayerFSMount` 的 reconcile 逻辑负责创建和维护：

- SlayerFS 挂载 `Deployment` 或 `DaemonSet`
- 可选的消费工作负载 `Deployment` 或 `DaemonSet`
- 挂载状态 `status`
- 对 `SlayerFSCluster`/ConfigMap 就绪性的等待与关联
...
- Kind: `SlayerFSMount`
- Group: `storage.slayerfs.io`
- Version: `v1alpha1`

`SlayerFSCluster` 最小示例：
...
`SlayerFSMount` 会引用一个同 namespace 下的 `SlayerFSCluster`，并复用该 cluster 生成的 `ConfigMap`。

当前挂载工作负载可以声明为 `Deployment` 或 `DaemonSet`，其特点是：

- 使用 `privileged` + `SYS_ADMIN`
- 挂载宿主机 `/dev/fuse`
- 在容器内运行 `slayerfs mount --config ...`
- 默认用 `emptyDir` 作为 `/var/lib/slayerfs` 状态目录，也可以通过 `statePvcName` 绑定已有 PVC
...
spec:
  ...
  image: slayerfs:local
  hostMountPath: /var/lib/slayerfs/mounts/demo
...
- 一个挂载 workload，把 SlayerFS 挂到宿主机 `/var/lib/slayerfs/mounts/demo`
...
- 让 `SlayerFSMount` 管理 sidecar / 业务 Pod 模板
- 让 `SlayerFSMount` 直接接收更完整的 Deployment / StatefulSet 模板片段
- 增加 `SlayerFSNodeRuntime` 做节点级挂载
- 或者演化成 CSI 相关的控制面组件
