...
`slayerfs-operator` 是一个独立于 `slayerfs` 主程序的控制面工程。

它的目标不是把 Kubernetes 能力硬塞进 `slayerfs mount` 二进制，而是提供一个单独的 operator 来管理：
...
- `SlayerFSCluster`
  ...
  - 负责 RustFS
  - 负责 bucket 初始化
  - 负责生成 SlayerFS `config.yaml`
- `SlayerFSMount`
  - 负责启动真正执行 `slayerfs mount` 的 workload
  - 可选把挂载点暴露到宿主机目录
...
- 一个 watch `SlayerFSCluster`
- 一个 watch `SlayerFSMount`
...
## `SlayerFSCluster` reconcile 产物

一个 `SlayerFSCluster` 当前会生成以下资源：
...
`SlayerFSMount` 不会自己拼 Redis/S3 配置，而是引用 `SlayerFSCluster` 生成的配置。

## `SlayerFSMount` reconcile 产物

一个 `SlayerFSMount` 当前最多会生成两类 workload：
...
1. `SlayerFSCluster` 创建 Redis / RustFS / ConfigMap
2. `SlayerFSMount` 等待对应 cluster 和 config 就绪
