...
- RustFS Pod 已启动
- bucket 初始化 `Job` 已完成
- `SlayerFSCluster.status.configMap` 已出现

如果这一步没有完成，`SlayerFSMount` 只会不断等待，不会真正进入运行态。

## 流程 2：仅创建挂载 workload

适合：

- 调试 `slayerfs mount` 本身
- 验证 FUSE 与后端链路
...
- `SlayerFSCluster`
- `SlayerFSMount.workloadKind = Deployment`
- 不开 `hostMountPath`
- 不开 `consumer`
...
- `SlayerFSMount.workloadKind = DaemonSet`
- 开 `hostMountPath`
- `mountPropagation = Bidirectional`
...
## 1. `SlayerFSMount` 一直 `Pending`

优先检查：

- `clusterRef.name` 是否存在
- `SlayerFSCluster` 是否已产生 `ConfigMap`
- namespace 是否一致
...
1. `kubectl get slayerfscluster,slayerfsmount -o yaml`
