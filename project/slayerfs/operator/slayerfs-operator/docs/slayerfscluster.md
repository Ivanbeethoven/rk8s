...
`SlayerFSCluster` 表示一套可供 SlayerFS 挂载平面使用的后端依赖。
...
该配置当前主要服务于 `SlayerFSMount`。

## 生成的 `ConfigMap`

`SlayerFSCluster` reconcile 的关键产物之一是：

- `<cluster-name>-slayerfs-config`
