...
- `SlayerFSCluster` 更偏后端编排
- `SlayerFSMount` 更偏运行时编排

## 与 `SlayerFSCluster` 的关系

`SlayerFSMount.spec.clusterRef.name` 必须指向同 namespace 下的一个 `SlayerFSCluster`。

当前 reconcile 流程会依次检查：

1. `SlayerFSCluster` 是否存在
2. 该 cluster 对应的 `ConfigMap` 是否存在
3. 满足后才创建挂载 workload

如果这些前置条件不满足：

- `SlayerFSMount` 不会直接失败
- 会停留在 `Pending`
...
- `replicas: 1`
- `mountPath: /mnt/slayerfs`
- `configPath: /run/slayerfs/config.yaml`
- `statePath: /var/lib/slayerfs`
- `logLevel: slayerfs=info`
...
- `SlayerFSMount` 会显示为 `Progressing`
...
- 共享 SlayerFS 挂载仍来自同一个 `hostMountPath`
...
- `mountPath`
- 表示该 container 看到共享 SlayerFS 数据的路径
- 如果未设置，则回退到 `consumer.mountPath`
- `volumeMounts`
- 用于额外挂载 `consumer.volumes`
- 共享 SlayerFS 挂载本身由 operator 自动注入，不需要手写
...
```bash
slayerfs mount --config <configPath> <mountPath>
```
...
它是一个普通 workload，不带特权设置，不运行 SlayerFS，只消费宿主机路径。
...
spec:
  ...
  consumer:
    ...
    mountPath: /data
    podLabels:
      app.kubernetes.io/part-of: slayerfs-demo
    initContainers:
      - name: init-check
        image: busybox:1.36
        command:
...
- 所谓“接管已有 workload 模板”，当前含义是把模板结构迁移到 `SlayerFSMount.spec.consumer`，由 operator 再生成新的 Deployment / StatefulSet
