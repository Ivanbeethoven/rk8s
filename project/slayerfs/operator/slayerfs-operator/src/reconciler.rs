...
use crate::crd::{
    ...
    MountPropagationMode, MountToleration, MountWorkloadKind, SlayerFSCluster,
    SlayerFSClusterStatus, SlayerFSMount, SlayerFSMountStatus,
};
...
pub async fn reconcile_cluster(
    cluster: Arc<SlayerFSCluster>,
    ctx: Arc<OperatorContext>,
) -> Result<Action, ReconcileError> {
    let client = ctx.client.clone();
    let namespace = cluster
        .namespace()
        .ok_or_else(|| anyhow!("SlayerFSCluster must be namespaced"))?;
    let owner = cluster
    ...
    apply_slayerfs_config(&client, &namespace, &cluster, &owner).await?;
...
pub async fn reconcile_mount(
    mount: Arc<SlayerFSMount>,
    ctx: Arc<OperatorContext>,
) -> Result<Action, ReconcileError> {
    let client = ctx.client.clone();
    let namespace = mount
        .namespace()
        .ok_or_else(|| anyhow!("SlayerFSMount must be namespaced"))?;
    let owner = mount
        .controller_owner_ref(&())
        .ok_or_else(|| anyhow!("failed to build owner reference"))?;
    ...
    let cluster_api: Api<SlayerFSCluster> = Api::namespaced(client.clone(), &namespace);
    let config_api: Api<ConfigMap> = Api::namespaced(client.clone(), &namespace);

    let Some(cluster) = cluster_api
        .get_opt(&cluster_name)
        .await
        .with_context(|| format!("load SlayerFSCluster {cluster_name}"))?
    else {
        patch_mount_status(
            &client,
            &namespace,
            &mount,
            "Pending",
            &format!("waiting for SlayerFSCluster {cluster_name}"),
            None,
...
fn cluster_config_map_name(cluster_name: &str) -> String {
    format!("{cluster_name}-slayerfs-config")
}
...
async fn apply_slayerfs_config(
    client: &kube::Client,
    namespace: &str,
    cluster: &SlayerFSCluster,
    owner: &OwnerReference,
...
fn build_mount_pod_template(
    ...
    let mount_command = format!(
        "mkdir -p {mount_path} {state_path} && exec /usr/local/bin/slayerfs mount --config {config_path} {mount_path}",
    ...
    let mut pod_annotations = BTreeMap::from([
        (
            "storage.slayerfs.io/cluster".to_string(),
            cluster_name.clone(),
        ),
        (
            "storage.slayerfs.io/config-map".to_string(),
            config_map_name.to_string(),
        ),
    ]);
    if let Some(generation) = cluster.metadata.generation {
        pod_annotations.insert(
            "storage.slayerfs.io/cluster-generation".to_string(),
    ...
    if let Some(host_mount_path) = &mount.spec.host_mount_path {
        pod_annotations.insert(
            "storage.slayerfs.io/host-mount-path".to_string(),
            host_mount_path.clone(),
        );
        pod_annotations.insert(
            "storage.slayerfs.io/mount-propagation".to_string(),
        ...
        spec: Some(PodSpec {
            ...
            containers: vec![Container {
                ...
                env: Some(vec![
                    ...
                    EnvVar {
                        name: "SLAYERFS_CONFIG_PATH".to_string(),
                        value: Some(mount.spec.config_path.clone()),
                        ..EnvVar::default()
                    },
                    EnvVar {
                        name: "SLAYERFS_HOME".to_string(),
                        value: Some(mount.spec.state_path.clone()),
                        ..EnvVar::default()
                    },
                    EnvVar {
                        name: "SLAYERFS_MOUNT_POINT".to_string(),
...
fn build_consumer_pod_template(
    ...
    annotations.insert(
        "storage.slayerfs.io/consumer-host-mount-path".to_string(),
...
fn mount_state_volume(mount: &SlayerFSMount) -> Volume {
...
fn render_config(cluster: &SlayerFSCluster) -> String {
    let cluster_name = cluster.name_any();
...
async fn patch_cluster_status(
    ...
    let api: Api<SlayerFSCluster> = Api::namespaced(client.clone(), namespace);
    let cluster_name = cluster.name_any();
    let status = SlayerFSClusterStatus {
    ...
    let patch = json!({
        "apiVersion": "storage.slayerfs.io/v1alpha1",
        "kind": "SlayerFSCluster",
        "status": status,
    });
    api.patch_status(
        &cluster_name,
        &PatchParams::apply("slayerfs-operator").force(),
        &Patch::Apply(&patch),
    )
    .await
...
async fn patch_mount_status(
    ...
    let api: Api<SlayerFSMount> = Api::namespaced(client.clone(), namespace);
    let mount_name = mount.name_any();
    let status = SlayerFSMountStatus {
