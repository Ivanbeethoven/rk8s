...
本目录提供一套与 `docker/compose-xfstests` 对齐的 Kubernetes 版本编排，用于在集群里运行 SlayerFS 的 xfstests。
...
- `rustfs-init-job.yaml`
  ...
  - 负责确保 `slayerfs-data` bucket 存在
- `xfstests-job.yaml`
  - 对应 compose 里的 `xfstests`
  - 负责实际挂载 SlayerFS 并运行 xfstests
- `pvc.yaml`
...
```bash
bash k8s/run_xfstests/run_redis_xfstests.sh --image <your-registry>/slayerfs-xfstests:tag
```

当前脚本固定使用 `rustfs` 作为对象存储，也就是固定 `SLAYERFS_DATA_BACKEND=s3`。
