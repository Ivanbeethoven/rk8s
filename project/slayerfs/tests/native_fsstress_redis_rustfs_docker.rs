...
async fn run_phase(
    fs: Arc<VFS<ObjectBlockStore<S3Backend>, slayerfs::MetaClient<Arc<slayerfs::RedisMetaStore>>>>,
    worker_count: usize,
...
async fn test_native_fsstress_013_redis_rustfs_docker() {
    if std::env::var("SLAYERFS_REDIS_RUSTFS_DOCKER_TEST")
        .ok()
        .as_deref()
        != Some("1")
    {
        eprintln!("skip redis/rustfs docker test: set SLAYERFS_REDIS_RUSTFS_DOCKER_TEST=1");
        return;
    }

    let suffix = format!("{}-{}", std::process::id(), free_port());
    let redis_name = format!("slayerfs-redis-{suffix}");
    let rustfs_container_name = format!("slayerfs-rustfs-{suffix}");
    let network = format!("slayerfs-fsstress-{suffix}");
    let redis_port = free_port();
    let rustfs_port = free_port();
    let bucket = format!("slayerfs-fsstress-{suffix}");
