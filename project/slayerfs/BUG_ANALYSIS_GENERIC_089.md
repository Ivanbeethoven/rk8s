...
`xfstests generic/089` failed on SlayerFS because concurrent `t_mtab` processes could complete the mtab update loop but fail when appending their final status line to `mtab_output`.
...
```text
openat(AT_FDCWD, "/mnt/slayerfs/trace_out", O_WRONLY|O_CREAT|O_APPEND, 0666) = 3
...
```bash
echo first > /mnt/slayerfs/out
echo second >> /mnt/slayerfs/out
```
...
SlayerFS was interacting incorrectly with Linux FUSE writeback cache.
...
In `src/fuse/mod.rs`, when FUSE read is called with a file handle and the VFS returns `PermissionDenied`, SlayerFS now falls back to a temporary inode-based read handle:
...
```text
cargo fmt --check
cargo check -p rfuse3 -p slayerfs --tests
cargo test -p slayerfs vfs::fs::tests::io_tests::test_fs_append_handles_use_fresh_size_concurrently
```
...
```text
slayerfs/docker/compose-xfstests/artifacts/run-1777550125-27631
```
