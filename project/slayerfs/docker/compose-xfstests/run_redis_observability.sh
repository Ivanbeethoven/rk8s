...
validate_stats_file() {
    ...
    require_contains "$stats_file" "slayerfs_cache_requests_total"
    require_contains "$stats_file" "slayerfs_cache_hit_ratio"
    require_contains "$stats_file" "slayerfs_writeback_dirty_bytes"
    require_contains "$stats_file" "slayerfs_reader_buffer_bytes"
    require_contains "$stats_file" "slayerfs_s3_get_ops_total"
    require_contains "$stats_file" "slayerfs_s3_put_ops_total"
}

validate_xfstests_artifact() {
    local dir="$1"
    info "校验 xfstests artifact: $dir"
    require_file "$dir/slayerfs.log"
...
validate_perf_artifact() {
    ...
    require_contains "$dir/report.md" "## SlayerFS Stats"
