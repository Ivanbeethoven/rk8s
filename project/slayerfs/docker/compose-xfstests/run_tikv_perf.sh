...
选项:
  ...
  --namespace <NAME>         TiKV metadata key namespace，默认: slayerfs
...
BENCH_ARGS_VALUE=""
TIKV_NAMESPACE_VALUE="${SLAYERFS_META_TIKV_NAMESPACE:-slayerfs}"
...
run_slayerfs_bench() {
    ...
    info "运行宿主机 slayerfs_bench（tikv backend）"
    (
        cd "$PROJECT_DIR"
        env \
            RUST_LOG="${RUST_LOG:-warn}" \
            SLAYERFS_BENCH_META_BACKEND=tikv \
            SLAYERFS_BENCH_META_TIKV_PD_ENDPOINTS="$benchmark_pd_endpoints" \
            SLAYERFS_BENCH_META_TIKV_NAMESPACE="$TIKV_NAMESPACE_VALUE" \
...
export SLAYERFS_META_TIKV_NAMESPACE="$TIKV_NAMESPACE_VALUE"
if [[ "$USE_S3" == true ]]; then
