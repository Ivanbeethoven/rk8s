...
description:
  - run LTP filesystem tests inside docker container against slayerfs with redis metadata backend
  - object storage is rustfs (SLAYERFS_DATA_BACKEND=s3)
...
ts="$(date +%s)-$RANDOM"
PROJECT_NAME="slayerfs-ltp-redis-${ts}"
